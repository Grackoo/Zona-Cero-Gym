...
export default function Reports() {
  ...
  const recentTransactions = [
    { id: 'TRX-8923', date: 'Hoy, 14:32', client: 'Sarah Jenkins', type: 'Membresía Anual', amount: 450.00, status: 'Completado' },
    { id: 'TRX-8922', date: 'Hoy, 12:15', client: 'Marcus Johnson', type: 'Suplementos (Whey)', amount: 45.00, status: 'Completado' },
    { id: 'TRX-8921', date: 'Hoy, 09:05', client: 'Elena Rodriguez', type: 'Entrenamiento Personal', amount: 120.00, status: 'Pendiente' },
  ];

  return (
    <div className="h-full flex flex-col overflow-y-auto">
      <PageHeader title="Resumen Financiero" subtitle="Rastrea ingresos, gastos y rentabilidad general.">
        <div className="flex gap-3 ml-auto">
          <div className="flex bg-[#1e293b] rounded-lg p-1 border border-cero-border">
            ...
            <button className="px-4 py-1.5 text-sm text-cero-text-muted hover:text-white transition-colors flex items-center gap-1"><Calendar size={14} /> Personalizado</button>
          </div>
      ...
      <div className="p-8">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Tooltip 
                    ...
                    itemStyle={{ color: '#fff' }}
                  />
                  <Bar dataKey="revenue" fill="#d4ff00" radius={[4, 4, 0, 0]} name="Ingresos" />
            ...
            <div className="space-y-6">
               <div>
                 ...
                 <div className="h-2 w-full bg-[#1e293b] rounded-full overflow-hidden">
                   <div className="h-full bg-cero-lime w-[65%]"></div>
                 </div>
               </div>
               <div>
                 <div className="flex justify-between text-sm mb-2">
        ...
        <div className="bg-cero-panel border border-cero-border rounded-xl overflow-hidden flex flex-col">
          <div className="p-6 border-b border-cero-border flex justify-between items-center bg-[#10161c]">
             ...
             <button className="text-sm text-cero-lime hover:underline">Ver Todo</button>
          </div>
                ...
                {recentTransactions.map((trx, i) => (
                  <tr key={i} className="hover:bg-cero-bg/80 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${trx.status === 'Completado' ? 'bg-cero-lime/10 text-cero-lime border border-cero-lime/20' : 'bg-[#1e293b] text-gray-400 border border-cero-border'}`}>
                        {trx.status}
                      </span>
                    </td>
                  </tr>
